...
public class Kategori {
    ...
    public ArrayList<Kategori> getByNamaAndKeterangan(String nama, String keterangan) {
        ...
        if ((nama.trim().length()>0) && (keterangan.trim().length()>0)){
            rs = DBHelper.selectQuery("SELECT * FROM kategori Where nama = '"+ nama +"' and keterangan = '"+ keterangan+"'");         }else if ((nama.trim().length()>0) && (keterangan.trim().length()==0)){
                rs = DBHelper.selectQuery("SELECT * FROM kategori Where nama = '"+ nama+"'");
            }else {
                rs = DBHelper.selectQuery("SELECT * FROM kategori Where keterangan = '"+ keterangan+"'");         }                        try {             while (rs.next()) {                 Kategori kat = new Kategori();                 kat.setIdkategori(rs.getInt("idkategori"));                 kat.setNama(rs.getString("nama"));                 kat.setKeterangan(rs.getString("keterangan"));                 ListKategori.add(kat);             }         } catch (Exception e) {             e.printStackTrace();         }         return ListKategori;     } 
}